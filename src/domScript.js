...
function buildDomScript() {
    ...
    function scanBtn(b, candidates) {
        if (b.offsetParent === null) return;
        if (clicked.has(b)) return;
        ...
        for (var se = 0; se < EDITOR_SKIP.length; se++) {
            if (text.indexOf(EDITOR_SKIP[se]) === 0) return;
        }
        if (isInsideEditor(b)) return;

        // Check if text matches a known approval pattern
        var matched = false;
        ...
        // Safety: only add if a reject sibling confirms this is an approval dialog
        if (isApprovalButton(b)) {
            candidates.push({ el: b, text: text });
        }
    }
    ...
    setInterval(function() {
        ...
        // 1. Scan known containers first (notifications, dialogs, side panel)
        for (var ci = 0; ci < APPROVAL_CONTAINERS.length; ci++) {
            ...
            for (var cj = 0; cj < containers.length; cj++) {
                ...
                for (var bk = 0; bk < btns.length; bk++) {
                    scanBtn(btns[bk], candidates);
                }
            }
        }

        // 2. Fallback: scan entire body if known containers had no candidates
        //    (catches agent chat in editor tabs and future container changes)
        if (FALLBACK_CONTAINER && candidates.length === 0) {
            var allBtns = document.querySelectorAll(
                'button, a.action-label, [role="button"], .monaco-button'
            );
            for (var fb = 0; fb < allBtns.length; fb++) {
                scanBtn(allBtns[fb], candidates);
            }
        }

        if (candidates.length === 0) return;

        // 3. Prefer persistent approval (Always Allow, Allow This Conversation)
        var target = null;
        for (var k = 0; k < candidates.length && !target; k++) {
            for (var pp = 0; pp < PRIORITY_PATTERNS.length; pp++) {
                if (candidates[k].text === PRIORITY_PATTERNS[pp]) {
                    target = candidates[k];
                    break;
                }
            }
        ...
        console.log('[Supersmooth] Auto-click: ' + target.text);
        clicked.add(target.el);
        target.el.click();
    }, 1000);
...
// JS-safe markers for embedding inside workbench.desktop.main.js
const JS_DOM_TAG_START = '/* SUPERSMOOTH-DOM-START */';
const JS_DOM_TAG_END = '/* SUPERSMOOTH-DOM-END */';

function buildDomScriptForJs() {
    return `\n${JS_DOM_TAG_START}\n${buildDomScript()}\n${JS_DOM_TAG_END}`;
}

module.exports = { buildDomScript, buildDomScriptForJs, DOM_TAG_START, DOM_TAG_END, JS_DOM_TAG_START, JS_DOM_TAG_END };
