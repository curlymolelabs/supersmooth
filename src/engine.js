...
const { buildDomScript, buildDomScriptForJs, DOM_TAG_START, DOM_TAG_END, JS_DOM_TAG_START, JS_DOM_TAG_END } = require('./domScript');
...
function applyPatch(options = {}) {
    ...
    if (status.overallState === 'patched') {
        // JS bundles are already patched, but re-embed the DOM script in case
        // the Supersmooth extension version has been upgraded.
        const appRoot = appRootFromInstallRoot(status.basePath);
        const wbRecord = status.records.find(r => r.key === 'workbench');
        if (wbRecord) {
            let content = fs.readFileSync(wbRecord.path, 'utf8');
            // Strip old DOM script block if present
            const jsTagRegex = new RegExp(
                JS_DOM_TAG_START.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') +
                '[\\s\\S]*?' +
                JS_DOM_TAG_END.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'),
                'g'
            );
            content = content.replace(jsTagRegex, '');
            content += buildDomScriptForJs();
            fs.writeFileSync(wbRecord.path, content);
        }
        return {
            ok: true,
...
function verifyInstallation(options = {}) {
    ...
    // Verify DOM script is embedded in workbench JS
    const wbRecord = status.records.find(r => r.key === 'workbench');
    if (wbRecord && wbRecord.activeContent) {
        if (!wbRecord.activeContent.includes(JS_DOM_TAG_START)) {
            return {
                ok: false,
                code: 'dom-marker-missing',
                status,
                message: 'Supersmooth DOM script marker missing from workbench JS bundle.'
            };
        }
    }
