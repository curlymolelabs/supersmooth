...
3. Click the `...` menu at the top of the panel and choose **Install from VSIX...**.
4. Select `supersmooth-0.1.0.vsix`.
5. A modal dialog appears after a few seconds asking whether to enable now.
6. Click **Enable Now**.
7. Fully quit Antigravity and reopen it. A window reload is not enough.
8. Done. Patches are active.
...
If you prefer to download from the marketplace:
...
3. 点击面板顶部 `...` 菜单, 选择 **Install from VSIX...**。
4. 选择 `supersmooth-0.1.0.vsix`。
5. 几秒后弹出模态对话框, 询问是否立即启用。
6. 点击 **Enable Now**。
7. 完全退出并重新打开 Antigravity。仅重新加载窗口是不够的。
8. 完成。补丁已激活。
...
#### 方法 A: VSIX からインストール (推奨)

1. Antigravity を開く。
2. 拡張機能パネルを開く (Ctrl+Shift+X)。
3. パネル上部の `...` メニューから **Install from VSIX...** を選択。
4. `supersmooth-0.1.0.vsix` を選択。
5. 数秒後にモーダルダイアログが表示され、今すぐ有効にするか尋ねられます。
6. **Enable Now** をクリック。
7. Antigravity を完全に終了して再度開く。ウィンドウのリロードだけでは不十分です。
8. 完了。パッチが有効になりました。
...
1. Antigravity를 엽니다.
2. 확장 프로그램 패널을 엽니다 (Ctrl+Shift+X).
3. 패널 상단 `...` 메뉴에서 **Install from VSIX...** 를 선택합니다.
4. `supersmooth-0.1.0.vsix`를 선택합니다.
5. 몇 초 후 모달 대화 상자가 나타나 지금 활성화할지 묻습니다.
6. **Enable Now**를 클릭합니다.
7. Antigravity를 완전히 종료하고 다시 엽니다. 창 새로고침만으로는 충분하지 않습니다.
8. 완료. 패치가 활성화되었습니다.
...
1. [Open VSX](https://open-vsx.org/)의 Supersmooth 페이지를 방문합니다.
